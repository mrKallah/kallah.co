...
class_zero = "rand"  # the name of the random class, this is what it will be printed as
...
augment = True  # Whether or not to augment, if img_size or num_augment has change this must be true. Saves time not to re-augment every time you run the program if not needed.
...
def test_global_vars():
	"""
	Tests if the global variables are not illegal values. Exits if they are
	"""
	if image_size < 0:
		exit("image_size must be positive", -1)
	if num_channels != 1 | num_channels != 3:
		exit("Invalid channel number")
	if filter_size1 < 0:
		exit("Ifilter_size1 must be positive", -1)
	if num_filters1 < 0:
		exit("num_filters1 must be positive", -1)
	if filter_size2 < 0:
		exit("filter_size2 must be positive", -1)
	if num_filters2 < 0:
		exit("num_filters2 must be positive", -1)
	if fc_size < 0:
		exit("fc_size must be positive", -1)
	if optimization_iterations < 0:
		exit("optimization_iterations must be positive", -1)
	if print_and_save_regularity < 0:
		exit("print_and_save_regularity must be positive", -1)
	if print_and_save_regularity < 0:
		exit("print_and_save_regularity must be positive", -1)
	if num_classes < 0:
		exit("num_classes must be positive", -1)
	if plt_show != True:
		if plt_show != False:
			exit("plt_show must be a Boolean value", -1)
	if class_zero == class_one:
		exit("class_zero and class_one must be different", -1)
	if file_name_identifier == "":
		exit('file_name_identifier cannot be ""', -1)
	if batch_size < 0:
		exit("batch_size must be positive", -1)
	if train_batch_size < 0:
		exit("train_batch_size must be positive", -1)
	if train_data_directory == test_data_directory:
		exit("the test, train and validation directories must be different", -1)
	if train_data_directory == validation_data_directory:
		exit("the test, train and validation directories must be different", -1)
	if test_data_directory == validation_data_directory:
		exit("the test, train and validation directories must be different", -1)
	if augment != True:
		if augment != False:
			exit("augment must be a Boolean value", -1)
...
def load(saver, session, save_dir="checkpoints/", file_name="best_validation"):
	"""
...
def initiate():
	...
	data.test.images, data.test.labels = load_data(test_data_directory, file_name_identifier, image_size_flat,
	                                               num_channels)
	data.validation.images, data.validation.labels = load_data(validation_data_directory, file_name_identifier,
...
def main():
	...
	utils.plot_nine_images(images, class_one, class_zero, cls_true, plt_show, image_shape, channels=num_channels,
	...
	utils.print_test_accuracy(data, batch_size, x, y_true, session, y_pred_cls, class_one, class_zero, plt_show,
	...
	print("Calculating test accuracy")
	test_accuracy, cm = utils.print_test_accuracy(data, batch_size, x, y_true, session, y_pred_cls, class_one,
	                                              class_zero, plt_show, channels=num_channels,
	                                              confusion_matrix=confusion_matrix, img_shape=image_shape,
